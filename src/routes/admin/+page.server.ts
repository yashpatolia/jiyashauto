...
export const load: PageServerLoad = async () => {
	...
	const recentSubmissions = db
		.prepare('SELECT id, name, email, inquiry_type, message, created_at FROM contact_submissions ORDER BY created_at DESC LIMIT 5')
		.all() as { id: number; name: string; email: string; inquiry_type: string; message: string; created_at: string }[];
